...
void MyMesh::injectSensor(const char* hex_id) {
  ...
  mesh::Identity sensor_id(pub_key);

  ClientInfo* client = acl.putClient(sensor_id, PERM_ACL_ADMIN | 192);
  if (client) {
      Serial.println("[SETUP] Sensor successfully hardcoded into Gateway ACL!");
